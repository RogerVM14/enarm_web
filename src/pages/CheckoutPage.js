...
const CheckoutPage = () => {
    ...
    const successCallback = (res) => {
        ...
        */
        ...
        if (res) {
            ...
            navigate('/checkout_thankful', { replace: true });
        }
    };
    ...
    const handleSubmitPaymentInformation = async () => {
        ...
        */
        ...
        const { cardNumber, cvv, expiredDate, cardOwnerName } = paymentCheckoutiInfo;

        if (cardNumber === '' || cardNumber.length !== 16) {
            window.alert("Favor de introducir un numero de tarjeta valido.");
            return;
        }

        if (cvv === '') {
            window.alert("Favor de introducir el CVV de su tarjeta.")
            return;
        }

        if (expiredDate === '') {
            window.alert("Favor de introducir la fecha de vencimiento de su tarjeta.");
            return;
        }

        if (cardOwnerName === '') {
            window.alert("Favor de introducir su nombre tal como viene en su tarjeta.")
            return;
        }
