...
const CardLegacy = (props) => {
    ...
    return (
        <div className={`c-card-legacy ${deviceType}`}>
            <div className={`c-cardlegacy-header ${deviceType}`} onClick={() => { setShowBody(!showBody) }}>
                <div
                    className="title-wrapper"
                    style={isSmart ? { justifyContent: "space-between", width: "100%" } : {}}
                >
                    <span className={`medium-${isSmart ? "14" : "16"}`}>
            ...
            </div>
            {
                isSmart ? (
                    showBody ?
                        <div className={`c-cardlegacy-body ${deviceType}`}>
